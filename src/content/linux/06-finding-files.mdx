...
**第 1 题：找出所有 .py 或 .md 文件**

```bash
mkdir -p proj/src proj/docs
touch proj/src/a.py proj/src/b.py proj/docs/README.md proj/docs/todo.md proj/x.tmp

find proj \( -name '*.py' -o -name '*.md' \)
```

括号在 Shell 里需要转义（`\(` 与 `\)`）或者加引号，否则会被当成分组语法；`-o` 表示 OR。也可以写 `find proj -name '*.py' -o -name '*.md'`，但这样 `.tmp` 文件也会被列出来（OR 的优先级会破坏默认的 AND 语义），所以练习里特意强调用括号分组。

**第 2 题：大于 1M 且小于 10M**

```bash
truncate -s 500K  small.bin
truncate -s 2M    medium.bin
truncate -s 20M   big.bin

find . -maxdepth 1 -type f -size +1M -size -10M
# 输出 ./medium.bin
```

多个条件并列默认是 AND：`+1M` 排除 small.bin，`-10M` 排除 big.bin，只剩 2M 的 medium.bin。

**第 3 题：找出 30 天前的 .log 并删除（先预演再删）**

```bash
# 构造演示文件
touch -d '40 days ago' old1.log
touch -d '5 days ago'  recent.log
touch new.log

# 第一步：先打印确认列表（不带 -delete）
...
# 第三步：验证
find . -name '*.log'
# 只剩 recent.log 和 new.log
```

`-mtime +30` 表示"超过 30 天未修改"，`-delete` 必须放在所有条件**之后**。

**第 4 题：为什么 which 找不到 cd**

```bash
type cd    # cd is a shell builtin（Shell 内建命令）
type ls    # ls is /bin/ls（外部程序）
which cd   # 无输出，或报错 not found
```

`cd` 是 **Shell 内建命令**——它要改变当前 Shell 进程自己的工作目录，必须内建在 Shell 里，根本没有独立的可执行文件。`which` 只负责在 `PATH` 目录里找**外部程序文件**，所以找不到 cd。`ls` 是外部程序，`type ls` 会显示它的绝对路径，`which ls` 也能找到。
</Answer>
</Callout>
