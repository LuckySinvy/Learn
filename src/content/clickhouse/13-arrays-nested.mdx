...
FROM user_paths
ARRAY JOIN
    arraySlice(pages, 1, length(pages) - 1) AS from_page,   -- 错位一格
    arraySlice(pages, 2)                    AS to_page
GROUP BY from_page, to_page
ORDER BY transitions DESC
LIMIT 10;
...
`arraySlice(pages, 1, n-1)` 取除最后一个外的全部页面，`arraySlice(pages, 2)` 取除第一个外的全部页面，两个等长数组按位置对齐展开后就是「相邻页面对」。结果第一行就是最常见的跳转路径。
...
-- 插入 3 笔订单（每笔 2~4 个商品），各子数组必须等长
INSERT INTO order_products VALUES
    (1, [101, 205],          [2, 1],          [299.00, 89.00]),
    (2, [101, 205, 388],     [1, 2, 1],       [299.00, 89.00, 1299.00]),
    (3, [388, 101],          [1, 3],          [1299.00, 299.00]);
```

方式 A（ARRAY JOIN 展开后统计每个商品总销量）：
...
SELECT
    order_id,
    arraySum(arrayMap((q, p) -> q * p, items.qty, items.price)) AS order_total
FROM order_products;
```

再验证两种方式的总额一致：`SELECT sum(order_total) FROM (...方式B...)` 应等于 `SELECT sum(items.qty * items.price) FROM order_products ARRAY JOIN items`。
...
INSERT INTO events_map SELECT number, map('coupon', toString(number % 100), 'channel', 'wechat') FROM numbers(100000);

-- 查询方式一：直接读 Map
SELECT count() FROM events_map WHERE props['coupon'] = '42';

-- 提升为独立列
ALTER TABLE events_map ADD COLUMN coupon String MATERIALIZED props['coupon'];
ALTER TABLE events_map MATERIALIZE COLUMN coupon;

-- 查询方式二：读独立列
SELECT count() FROM events_map WHERE coupon = '42';
```

对比两者的 `Elapsed` 和 `Processed bytes`（查询输出末尾会打印）：方式一每行都要读整个 Map 列再在内存里查找；方式二只读一列定长字符串。当 Map 键很多、访问很频繁时，独立列通常快数倍、读的字节少 90% 以上。
...
WITH user_seqs AS (
    ...
    FROM (
        SELECT user_id, event_type, duration_ms, event_time
        FROM events ORDER BY user_id, event_time
    )
    GROUP BY user_id
)
SELECT
    user_id,
    arrayFilter((t, d) -> d > 3000, types, durations) AS slow_events
FROM user_seqs
WHERE arrayCount((t, d) -> d > 3000, types, durations) > 0
LIMIT 5;
```

`arrayFilter` 的 lambda 接收两个并行数组，`d > 3000` 为真的位置保留 `types` 里的对应元素，即得到所有慢事件类型列表。

5. JSONExtract vs MATERIALIZED 列：
...
INSERT INTO events_json SELECT now() - number, '{"order":{"id":9527,"amount":299.5},"active":true}' FROM numbers(100000);

-- 方式一：JSONExtract 每次解析
SELECT count() FROM events_json WHERE JSONExtractUInt(payload, 'order', 'id') = 9527;

-- 方式二：MATERIALIZED 列
ALTER TABLE events_json ADD COLUMN order_id UInt64 MATERIALIZED JSONExtractUInt(payload, 'order', 'id');
ALTER TABLE events_json MATERIALIZE COLUMN order_id;

SELECT count() FROM events_json WHERE order_id = 9527;
```

方式二在写入时就把 `order_id` 解析并单独落盘，查询变成普通列过滤，可建索引；方式一每次查询都要读整个 JSON 字符串并解析。数据量大时两者差距可达一个数量级。这就是「schema-on-write」：把已知高频字段提前建成正式列，只把真正动态的部分留在 JSON。
</Answer>
</Callout>
