...
<Callout type="tip" title="🤖 AI 时代，为什么还要学 Redis？">
AI 能帮你写一段 SET/GET 代码，却不知道你的热点 key 会不会把实例打挂、大 key 删除会不会阻塞整条命令执行链路。Redis 作为高并发缓存与实时状态的基石，其数据结构选择与缓存策略（击穿、穿透、雪崩）决定了系统的稳定性上限。不懂这些底层，AI 给出的「能跑」的方案在生产洪峰下随时可能雪崩。
</Callout>
