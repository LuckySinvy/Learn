...
```bash
127.0.0.1:6379> SADD tags:article:1 "redis" "cache" "database"
(integer) 3
127.0.0.1:6379> SADD tags:article:1 "redis"
(integer) 0             # 已存在，自动去重，返回实际新增数
127.0.0.1:6379> SISMEMBER tags:article:1 "redis"
(integer) 1
127.0.0.1:6379> SMISMEMBER tags:article:1 "redis" "mysql"
1) (integer) 1
2) (integer) 0          # 6.2+ 批量判断
...
```bash
127.0.0.1:6379> SADD follow:ada  u2 u3 u4
(integer) 3
127.0.0.1:6379> SADD follow:bob  u3 u4 u5
(integer) 3

127.0.0.1:6379> SINTER follow:ada follow:bob     # 交集: 共同关注
1) "u3"
2) "u4"
127.0.0.1:6379> SUNION follow:ada follow:bob     # 并集
1) "u2"
2) "u3"
3) "u4"
4) "u5"
127.0.0.1:6379> SDIFF follow:ada follow:bob      # 差集: ada 关注而 bob 没关注的
1) "u2"
```

带 STORE 后缀的版本把结果存进新 key，便于缓存复用：

```bash
127.0.0.1:6379> SINTERSTORE common:ada:bob follow:ada follow:bob
(integer) 2
127.0.0.1:6379> EXPIRE common:ada:bob 300
(integer) 1             # 交集结果缓存 5 分钟
127.0.0.1:6379> SINTERCARD 2 follow:ada follow:bob
(integer) 2             # 7.0+ 只要交集的数量，不要成员，更省
```
...
"intset"
127.0.0.1:6379> SADD strs "a" "b"
(integer) 2
127.0.0.1:6379> OBJECT ENCODING strs
"listpack"
127.0.0.1:6379> SADD strs "this-is-a-long-member-over-sixty-four-bytes-xxxxxxxxxxxxxxxxxxxxxxx"
(integer) 1
127.0.0.1:6379> OBJECT ENCODING strs
"hashtable"
```
...
```bash
127.0.0.1:6379> SADD like:post:42 u1
(integer) 1             # 返回 1 = 点赞成功
127.0.0.1:6379> SADD like:post:42 u1
(integer) 0             # 返回 0 = 已点过（可提示或执行取消逻辑）
127.0.0.1:6379> SCARD like:post:42
(integer) 1             # 点赞数
```
