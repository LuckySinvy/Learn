...
`avg by (instance)` 把每个实例的所有 CPU 核取平均，得到该实例整体使用率。
...
用 `histogram_quantile` 结合直方图的 `_bucket` 指标：
...
- 结果单位是秒，例如 `0.312` 表示 P95 = 312ms，可在面板 Standard options 里把单位设成 `s` 或 `ms`
- 多实例时先 `sum by (le)` 把分桶合并再算分位数，结果才是全量请求的 P95

**小提示：** 如果环境里没有直方图指标，P95 可以先按实例分别近似看趋势，但要下正式结论仍需要 histogram 数据。
</Answer>
</Callout>
