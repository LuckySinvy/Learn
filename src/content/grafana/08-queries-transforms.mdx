...
三个查询（查询 A / B / C，node-exporter 指标，按 `instance` 对齐）：
...
1. **Merge**：把三个查询结果按 `instance` 拼成一张宽表
2. **Organize fields**：重命名、调顺序、隐藏无用列，例如：
   - `Value #A` → `CPU 使用率`
   - `Value #B` → `内存使用率`
   - `Value #C` → `磁盘使用率`
   - 隐藏 `Time`、`__name__` 等列
3. **Sort by**：选字段 `磁盘使用率`，方向 **Descending（降序）**

**易错点：**

- CPU 查询里必须保留 `instance` 标签（用 `by (instance)`），Merge 才有共同的关联字段
- 若查询返回的是多个时间点的序列，先加一步 **Reduce**（Mode 选 `Last`）把每实例压成一行，再 Merge，效果更干净
- 结果里磁盘占用最高的实例排最上面，配合 Table 的阈值着色能一眼看到危险实例
</Answer>
</Callout>
