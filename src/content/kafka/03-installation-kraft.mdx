---
title: 安装与部署（KRaft 模式）
order: 3
description: 单机与三节点 KRaft 集群搭建、server.properties 关键配置与 Docker Compose 编排
---

# 安装与部署（KRaft 模式）

本章把 Kafka 跑起来：先单机快速体验，再搭一个贴近生产形态的三节点 KRaft 集群，最后给出 Docker Compose 一键编排。所有步骤基于 Kafka 3.7+，不需要 ZooKeeper。

## 1. 单机快速启动

### 1.1 下载与解压

Kafka 只需要 JDK 11+（推荐 17）：

```bash
# 确认 Java
java -version

# 下载并解压 (以 3.7.1 为例, Scala 2.13 构建)
curl -O https://archive.apache.org/dist/kafka/3.7.1/kafka_2.13-3.7.1.tgz
tar -xzf kafka_2.13-3.7.1.tgz
cd kafka_2.13-3.7.1
```

### 1.2 初始化并启动

KRaft 模式启动前必须**格式化存储目录**——生成集群 ID 并写入元数据：

```bash
# 1. 生成集群唯一 ID
KAFKA_CLUSTER_ID="$(bin/kafka-storage.sh random-uuid)"
echo $KAFKA_CLUSTER_ID

# 2. 用集群 ID 格式化日志目录
bin/kafka-storage.sh format -t $KAFKA_CLUSTER_ID \
  -c config/kraft/server.properties

# 3. 启动 (前台运行, 加 -daemon 可后台)
bin/kafka-server-start.sh config/kraft/server.properties
```

验证：

```bash
# 新开终端, 创建 topic 并收发一条消息
bin/kafka-topics.sh --bootstrap-server localhost:9092 \
  --create --topic hello --partitions 1 --replication-factor 1

echo "hello kafka" | bin/kafka-console-producer.sh \
  --bootstrap-server localhost:9092 --topic hello

bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 \
  --topic hello --from-beginning --max-messages 1
# 输出: hello kafka
```

<Callout type="warn" title="格式化只做一次">
`kafka-storage.sh format` 只在初始化时执行一次。对已有数据目录重复格式化会导致集群 ID 不一致、Broker 拒绝启动。如果启动报 InconsistentClusterIdException，通常是数据目录残留了旧集群的元数据。
</Callout>

## 2. server.properties 关键配置

单机默认配置能跑，但必须理解这些参数才能上生产：

```properties
############ 角色与节点 ############
# 该节点承担的角色: broker、controller 或两者兼任
process.roles=broker,controller
# 集群内唯一的节点 ID
node.id=1
# controller 仲裁投票者列表: id@host:port
controller.quorum.voters=1@localhost:9093

############ 监听器 ############
# 9092 服务客户端, 9093 用于 controller 内部通信
listeners=PLAINTEXT://:9092,CONTROLLER://:9093
# 客户端实际连接的地址 —— 容器/云环境最容易踩坑的配置
advertised.listeners=PLAINTEXT://localhost:9092
controller.listener.names=CONTROLLER
inter.broker.listener.name=PLAINTEXT

############ 存储 ############
# 日志数据目录, 生产环境用独立磁盘, 可逗号分隔多块盘
log.dirs=/var/kafka/data
# 默认保留 7 天
log.retention.hours=168
# 单个 segment 1GB
log.segment.bytes=1073741824

############ 主题默认值 ############
num.partitions=3
default.replication.factor=3
min.insync.replicas=2
# 生产环境务必关闭自动建 topic
auto.create.topics.enable=false
```

<Callout type="tip" title="advertised.listeners 是新手第一坑">
`listeners` 是进程绑定的地址，`advertised.listeners` 是**告诉客户端来连我用的地址**。客户端第一次连上任意 broker 后，会拿到集群元数据里的 advertised 地址再发起真正的读写连接。容器里如果 advertised 写的是容器内部主机名，宿主机客户端就会"能连上 bootstrap 却读写超时"。
</Callout>

## 3. 三节点 KRaft 集群

生产最小形态：3 个节点，每个都兼任 broker + controller。三份配置的差异只有 `node.id`、监听端口和日志目录：

```properties
# node1.properties (node2/node3 改 id、端口、目录即可)
process.roles=broker,controller
node.id=1
controller.quorum.voters=1@host1:9093,2@host2:9093,3@host3:9093
listeners=PLAINTEXT://:9092,CONTROLLER://:9093
advertised.listeners=PLAINTEXT://host1:9092
controller.listener.names=CONTROLLER
inter.broker.listener.name=PLAINTEXT
log.dirs=/var/kafka/data
default.replication.factor=3
min.insync.replicas=2
```

三台机器用**同一个集群 ID** 格式化并启动：

```bash
# 在一台机器上生成一次, 三台共用
KAFKA_CLUSTER_ID="$(bin/kafka-storage.sh random-uuid)"

# 每台机器分别执行
bin/kafka-storage.sh format -t $KAFKA_CLUSTER_ID -c node1.properties
bin/kafka-server-start.sh -daemon node1.properties
```

验证集群状态：

```bash
# 查看 controller 仲裁状态: 谁是 leader、各节点日志追赶进度
bin/kafka-metadata-quorum.sh --bootstrap-server host1:9092 describe --status

# 查看所有 broker
bin/kafka-broker-api-versions.sh --bootstrap-server host1:9092 | grep id
```

## 4. Docker Compose 编排

本地开发推荐直接用官方 `apache/kafka` 镜像。单节点版：

```yaml
# docker-compose.yml
services:
  kafka:
    image: apache/kafka:3.7.1
    ports:
      - "9092:9092"
    environment:
      KAFKA_NODE_ID: 1
      KAFKA_PROCESS_ROLES: broker,controller
      KAFKA_CONTROLLER_QUORUM_VOTERS: 1@kafka:9093
      KAFKA_LISTENERS: PLAINTEXT://:19092,CONTROLLER://:9093,EXTERNAL://:9092
      # 容器间用 kafka:19092, 宿主机用 localhost:9092
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka:19092,EXTERNAL://localhost:9092
      KAFKA_LISTENER_SECURITY_PROTOCOL_MAP: PLAINTEXT:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT
      KAFKA_CONTROLLER_LISTENER_NAMES: CONTROLLER
      KAFKA_INTER_BROKER_LISTENER_NAME: PLAINTEXT
      KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR: 1
      KAFKA_AUTO_CREATE_TOPICS_ENABLE: "false"
    healthcheck:
      test: ["CMD", "/opt/kafka/bin/kafka-broker-api-versions.sh",
             "--bootstrap-server", "localhost:19092"]
      interval: 10s
      timeout: 10s
      retries: 5
```

三节点版只需复制三份服务，`KAFKA_NODE_ID` 分别为 1/2/3，`KAFKA_CONTROLLER_QUORUM_VOTERS` 写全三个节点：

```yaml
      KAFKA_CONTROLLER_QUORUM_VOTERS: 1@kafka1:9093,2@kafka2:9093,3@kafka3:9093
```

启动与验证：

```bash
docker compose up -d
docker compose ps            # 等 healthcheck 变 healthy
docker exec -it <容器名> /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:19092 --list
```

## 5. 生产部署检查清单

| 项目 | 建议 |
|---|---|
| JDK | 17，堆 6-8GB（`KAFKA_HEAP_OPTS`），剩余内存留给 PageCache |
| 磁盘 | 数据目录独立磁盘，XFS，SSD 或多块 HDD |
| 文件句柄 | `ulimit -n 100000` 以上 |
| swap | `vm.swappiness=1` |
| 副本 | `default.replication.factor=3`，`min.insync.replicas=2` |
| 自动建 topic | 关闭 |
| controller | 大集群独立部署 3 或 5 个 controller 节点 |

<Callout type="warn" title="常见启动故障">
1. **端口被占**：9092/9093 冲突，`lsof -i :9092` 排查。
2. **Cluster ID 不匹配**：换配置重搭时没清空 `log.dirs`，删除数据目录或重新 format。
3. **客户端连不上**：99% 是 `advertised.listeners` 配错，用 `kafka-broker-api-versions.sh` 从客户端所在网络测试。
4. **controller 无法选主**：`controller.quorum.voters` 三台配置不一致，或防火墙拦了 9093。
</Callout>

## 小结

- KRaft 启动三步：生成集群 ID → format 存储目录 → 启动
- `process.roles`、`controller.quorum.voters`、`advertised.listeners` 是 KRaft 三大核心配置
- 三节点集群共用一个集群 ID，配置只差 node.id 与地址
- 本地开发用官方镜像 + Docker Compose，healthcheck 用 broker-api-versions 探测
- 生产记住：副本 3、min.insync.replicas 2、关自动建 topic、留内存给 PageCache

<Callout type="exercise" title="练习">
1. 用 Docker Compose 启动三节点集群，创建一个 3 分区 3 副本的 topic，用 describe 确认 leader 分布在三个不同 broker 上。
2. `docker stop` 掉其中一个 broker，再次 describe，观察 Leader 和 Isr 列的变化；重新启动后再看 Isr 是否恢复。
3. 故意把 advertised.listeners 改成错误主机名，体验一下客户端"能 bootstrap 却发不了消息"的经典故障。

<Answer>
**参考答案：**

**第 1 题 · 三节点集群 + 3 分区 3 副本**

完整的 compose 文件（三份服务只差 `KAFKA_NODE_ID` 和监听地址）：

```yaml
# docker-compose.yml
services:
  kafka1:
    image: apache/kafka:3.7.1
    hostname: kafka1
    ports: ["9092:9092"]
    environment: &base
      KAFKA_PROCESS_ROLES: broker,controller
      KAFKA_CONTROLLER_QUORUM_VOTERS: 1@kafka1:9093,2@kafka2:9093,3@kafka3:9093
      KAFKA_LISTENER_SECURITY_PROTOCOL_MAP: PLAINTEXT:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT
      KAFKA_CONTROLLER_LISTENER_NAMES: CONTROLLER
      KAFKA_INTER_BROKER_LISTENER_NAME: PLAINTEXT
      KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR: 3
      KAFKA_TRANSACTION_STATE_LOG_REPLICATION_FACTOR: 3
      KAFKA_TRANSACTION_STATE_LOG_MIN_ISR: 2
      KAFKA_AUTO_CREATE_TOPICS_ENABLE: "false"
      KAFKA_NODE_ID: 1
      KAFKA_LISTENERS: PLAINTEXT://:19092,CONTROLLER://:9093,EXTERNAL://:9092
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka1:19092,EXTERNAL://localhost:9092
  kafka2:
    image: apache/kafka:3.7.1
    hostname: kafka2
    ports: ["9094:9094"]
    environment:
      <<: *base
      KAFKA_NODE_ID: 2
      KAFKA_LISTENERS: PLAINTEXT://:19092,CONTROLLER://:9093,EXTERNAL://:9094
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka2:19092,EXTERNAL://localhost:9094
  kafka3:
    image: apache/kafka:3.7.1
    hostname: kafka3
    ports: ["9096:9096"]
    environment:
      <<: *base
      KAFKA_NODE_ID: 3
      KAFKA_LISTENERS: PLAINTEXT://:19092,CONTROLLER://:9093,EXTERNAL://:9096
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka3:19092,EXTERNAL://localhost:9096
```

注意 `KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR` 必须从单机版的 1 改成 3，否则内部 topic `__consumer_offsets` 只有单副本，一台挂掉就丢消费位点。

```bash
docker compose up -d
docker compose ps    # 三个都 running 后再操作

docker exec -it kafka1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:19092 \
  --create --topic orders --partitions 3 --replication-factor 3

docker exec -it kafka1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:19092 --describe --topic orders
```

期望输出（leader 均匀打散在 1/2/3 上，Isr 三个都齐）：

```text
Topic: orders  PartitionCount: 3  ReplicationFactor: 3
  Partition: 0  Leader: 1  Replicas: 1,2,3  Isr: 1,2,3
  Partition: 1  Leader: 2  Replicas: 2,3,1  Isr: 2,3,1
  Partition: 2  Leader: 3  Replicas: 3,1,2  Isr: 3,1,2
```

Kafka 建 topic 时会按 round-robin 轮流指定 leader，所以三个分区的 leader 自然分散——这正是"分区是并行单位"的体现：三台机器同时承担写入压力。

**第 2 题 · 停掉一个 broker 观察 Leader / Isr**

```bash
docker stop kafka2
# 等几秒让 controller 检测到失联（受 zk-less 模式下的 broker session 超时控制）
docker exec -it kafka1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:19092 --describe --topic orders
```

现象：

```text
  Partition: 0  Leader: 1  Replicas: 1,2,3  Isr: 1,3      <- 2 掉出 ISR
  Partition: 1  Leader: 3  Replicas: 2,3,1  Isr: 3,1      <- leader 从 2 切到 3
  Partition: 2  Leader: 3  Replicas: 3,1,2  Isr: 3,1
```

要观察到的三件事：

1. **Replicas 列不变**：副本分配是静态元数据，机器挂了副本"名单"还在。
2. **Isr 列少了 2**：ISR 是"当前追得上 leader 的副本集合"，是动态的。
3. **原本 leader 在 2 的分区自动切主**：controller 从 ISR 里选一个新 leader，这就是自动故障转移。整个过程客户端只会短暂报 `NotLeaderOrFollowerException` 然后自动刷新元数据重试。

重启后：

```bash
docker start kafka2
sleep 15
docker exec -it kafka1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:19092 --describe --topic orders
```

- **Isr 会恢复成 1,2,3**：broker 2 重启后作为 follower 从 leader 拉取落后的数据，追平（进入 `replica.lag.time.max.ms` 阈值内）后重新加入 ISR。
- 但 **Leader 不会自动切回去**，分区 1 的 leader 仍留在 3。这会导致 leader 分布不均。如果 `auto.leader.rebalance.enable=true`（默认开启），后台线程会在 `leader.imbalance.check.interval.seconds`（默认 300 秒）后自动平衡回来；也可以手动触发：

```bash
docker exec -it kafka1 /opt/kafka/bin/kafka-leader-election.sh \
  --bootstrap-server localhost:19092 --election-type PREFERRED --all-topic-partitions
```

"preferred leader"就是 Replicas 列表里的第一个副本。

**第 3 题 · advertised.listeners 配错的经典故障**

把 kafka1 的配置改成一个根本不存在的主机名：

```yaml
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka1:19092,EXTERNAL://wrong-host:9092
```

重启后从宿主机连 `localhost:9092`，你会看到：

```text
# 元数据能拿到（bootstrap 成功）
$ bin/kafka-topics.sh --bootstrap-server localhost:9092 --list
orders

# 但生产/消费卡住直至超时
$ bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic orders
> hello
ERROR Error when sending message to topic orders with key: null, value: 5 bytes
org.apache.kafka.common.errors.TimeoutException: Topic orders not present in
metadata after 60000 ms.
WARN [Producer clientId=console-producer] Error connecting to node
wrong-host:9092 (id: 1 rack: null) java.net.UnknownHostException: wrong-host
```

**故障原理**（这是 Kafka 排障必须刻进肌肉记忆的一条）：

```text
1. 客户端拿 bootstrap.servers 里的地址 → 连上任意一个 broker
2. 该 broker 返回集群元数据，里面写的是每个 broker 的 advertised.listeners
3. 客户端【丢弃】bootstrap 地址，改用元数据里的地址去连真正的分区 leader
   ↑ 第 3 步用的是 advertised.listeners，配错就连不上
```

所以 `bootstrap.servers` 只是"问路的入口"，真正干活用的是 broker 自报的地址。判断标志很明确：**`--list` 能成功但收发消息超时，且日志里出现的主机名不是你填的那个** —— 100% 是 `advertised.listeners` 的问题。

排查手段：

```bash
# 这个命令会打印客户端实际解析到的 broker 地址
bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092 | head -5
```

Docker / K8s / 云主机场景下最容易踩：容器内外、内网外网需要不同地址，正确做法是**配置多个 listener**（如示例中的 `PLAINTEXT` 给容器间通信、`EXTERNAL` 给宿主机），而不是试图用一个地址糊弄两边。
</Answer>
</Callout>
