...
SELECT o.id AS order_id, o.order_no, o.status, o.created_at,
...
-- 之后五表 JOIN 一行搞定
SELECT * FROM v_order_detail WHERE username = 'alice';
```
...
SELECT order_no, user_id, total_amount,
       ...
       total_amount / SUM(total_amount) OVER (PARTITION BY user_id) AS pct
FROM orders;
```

```text
+---------------+---------+--------------+------------+--------+
| order_no      | user_id | total_amount | user_total | pct    |
+---------------+---------+--------------+------------+--------+
| SO20260701001 |       1 |      8127.00 |    8304.00 | 0.9787 |
| SO20260703003 |       1 |       177.00 |    8304.00 | 0.0213 |
| SO20260702002 |       2 |      3999.00 |    3999.00 | 1.0000 |
| SO20260704004 |       3 |      8999.00 |    8999.00 | 1.0000 |
+---------------+---------+--------------+------------+--------+
```
...
+---------------+---------+---------+-----+-----------+
| ThinkPad X1   | 9999.00 |       1 |   1 |         1 |
| MacBook Air M3| 8999.00 |       2 |   2 |         2 |
| iPhone 15 Pro | 7999.00 |       3 |   3 |         3 |
| Xiaomi 14     | 3999.00 |       4 |   4 |         4 |
| 高性能MySQL    |  128.00 |       5 |   5 |         5 |
| SQL必知必会    |   49.00 |       6 |   6 |         6 |
+---------------+---------+---------+-----+-----------+
```
