...
db.posts.aggregate([
  ...
  { $set: { avgLikes: { $round: ["$avgLikes", 1] } } },
  { $sort: { totalViews: -1 } },
  { $limit: 10 }
])
```

注意 `$group` 输出顺序不确定，排序必须显式做；`$match` 放最前让它能走索引。
...
`$unwind` 把含 N 个标签的帖子拆成 N 条、每条一个标签，之后按标签分组计数即可。
...
db.users.aggregate([
  { $group: {
      _id: {
        city: "$profile.city",
        month: { $dateToString: { date: "$createdAt", format: "%Y-%m", timezone: "Asia/Shanghai" } }
      },
      count: { $sum: 1 }
  } },
  { $sort: { _id: 1 } }
...
**不需要 `$lookup`。** `$lookup` 是跨集合关联用的，而「用户的城市」和「用户的创建时间」都在 `users` 集合的同一个文档里，只对 `users` 单集合做复合分组即可。

**四、对比 `$match` 在 `$group` 前后的执行时间**
...
db.posts.aggregate([
  { $group: { _id: "$authorId", views: { $sum: "$stats.views" } } },
  { $match: { _id: "alice" } }
]).explain("executionStats")   // 注意要放在 aggregate 调用上
...
用 `explain("executionStats")` 看 `executionTimeMillis` 与各阶段处理的文档数：写法 A 先对全集合分组（`$group` 无法用索引且消耗大量内存），再过滤掉绝大部分组；写法 B 先用 `$match` 把集合缩小到 alice 一个人的帖子（可走 `authorId` 索引），再分组。数据量越大差距越明显。只有位于管道最前面的 `$match` 才能用索引。

**五、浏览数分档 S/A/B/C 并加每档平均点赞**
...
先 `$unwind` 展开、按标签分组去重（`_id: "$tags"` 使每个不同标签成为一组），最后 `$count` 统计组数，结果形如 `[ { "distinctTags": 128 } ]`。
</Answer>
</Callout>
