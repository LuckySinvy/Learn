...
<Callout type="tip" title="🤖 AI 时代，为什么还要学 Git？">
AI 帮你写代码，但生成的上百行改动能不能放心合进主干，仍然要靠版本控制来审查、追溯和回滚。Git 提供的不是"存历史"这么简单，而是团队协作的可追溯性与信任——每一行是谁、为什么改的，都清清楚楚。把 AI 产出的代码交给 Git 管理，你才既享受了速度，又守住了工程纪律。
</Callout>
...
`git log --oneline` 会从最新到最旧列出三条提交，最新的一条在最上面。`HEAD -> main` 会指向那一条最新的提交（第三次提交），表示 `main` 分支和当前所在位置 `HEAD` 都落在这条提交上。
...
`git hist` 等价于 `git log --oneline --graph`，与普通 `git log` 相比：每行只显示一条提交（`--oneline`），并用 `*` 和连线画出分支拓扑。在有多个分支或合并过的仓库里，能直观看到提交之间的分叉与合并关系，而 `git log` 默认按时间线性排列。

**3. 一句话解释**

因为 SVN 是集中式的，提交需要把差异上传到中央服务器，断网时本地没有完整历史，根本没法提交；而 Git 是分布式的，每个人的克隆都是完整仓库，提交只是在本地快照链上新增一个节点，不依赖任何服务器，所以断网也能提交。

**4. 思考题**

如果 Git 记录的是"差异"，要回到 500 个提交之前的版本，就必须从起点开始把 500 个补丁**顺序、逐个地反向应用**，中间任何一个补丁因为上下文变化而应用失败都会卡住，耗时还随历史长度线性增长。快照模型则不同：每个提交都完整保存了当时所有文件的状态（未变的文件只是指向旧快照的链接），切回去只需要把 HEAD 指向那个提交并更新工作区，与中间隔了多少个提交无关。这就是 Git 选择快照模型的原因——快、稳、每个版本都能独立还原。
</Answer>
</Callout>
