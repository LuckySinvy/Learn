...
```http title="模拟 HPACK：重复头从几百字节变一字节"
python3 -c '
...
**1. 构造并解析 RST_STREAM 帧**

用 Playground 1 的 `build_frame` 构造 RST_STREAM 帧，载荷是 4 字节大端错误码：

```python
# RST_STREAM 类型 0x3，载荷 4 字节错误码（大端）
rst = build_frame(0x3, 0x0, 1, struct.pack(">I", 0x8))   # 0x8 = CANCEL
length, ftype, flags, sid, payload = parse_frame(rst)
code = int.from_bytes(payload, "big")
print("帧:", ftype, "| 流ID:", sid, "| 错误码:", hex(code))
```

解析器要识别类型 0x3 后，把 4 字节载荷按大端读成错误码（`FRAME_TYPES` 里已注册 `3: "RST_STREAM"`）。RST_STREAM 的作用是**取消单个流**而不影响整个连接——用户取消下载时，HTTP/1.1 只能断整条连接，HTTP/2 只需发一个 RST_STREAM。

**2. 变化值的头拉低压缩率**

给每次请求加 `x-request-id: <每次不同的随机值>`：
...
def hpack_size(kv):
    ...
    dynamic_table[kv] = len(dynamic_table) + 62
    k, v = kv
    return 2 + len(k) + len(v)
```

由于 `x-request-id` 每次都是新值，**永远无法命中动态表**，每个请求都要按字面量发送（2 + 长度字节），压缩率显著下降（可能从省 90% 掉到省 50% 左右，取决于变化头的比例与长度）。

原因：**HPACK 动态表缓存的是"键值对"**，只有相同的完整头（键+值）才能用 1 字节索引代替；每次变化的值无法索引，只能全量编码。所以高频变化头（request-id、trace-id）对头部压缩"免疫"，动态表对稳定的大头（UA、Cookie）收益最大。

**3. 动态表为什么要同一连接内共享**

动态表是连接级状态：**发送方（客户端或服务器）在 HPACK 上下文里维护一张与对端同步的表**，双方按同一规则增删条目，之后发送方发 1 字节索引、接收方查自己的表就能还原完整头。这个同步协议（索引插入、表容量调整）是**有状态的**，只能在同一条连接里持续生效。

域名分片会破坏它：分片后同一域名内容分散到多条连接，每条连接各自维护独立的动态表——`User-Agent`、`Cookie` 等大头的索引在连接 A 里建了，连接 B 里完全没用，每个请求还得全量重发。表的大小还受 SETTINGS_HEADER_TABLE_SIZE 限制，连接越多，重复建立表的浪费越大。**多路复用 + 单连接才能把动态表压满**，这是"分片反模式"的第二个理由（第一个是拆散流）。

**4. 弱网下 HTTP/2 反而更慢**

解释：HTTP/2 把所有流挤在**一条 TCP 连接**上。TCP 保证字节严格有序——一个包丢了，它后面的所有字节都要等重传，**所有流一起罚站**（TCP 队头阻塞，第 13 章末尾）。地铁弱网丢包率高时：

- HTTP/1.1 开 6 条连接，丢包只堵其中 1 条，其余 5 条照跑；
- HTTP/2 单连接，任何一条流的丢包都会卡住整条连接上的全部资源。

验证思路：

1. 用 `tc`（netem）或 Charles/Network Link Conditioner 模拟高丢包（如 5%-10%）+ 高延迟环境；
2. 分别用 HTTP/1.1 和 HTTP/2 加载同一页面，对比首屏时间与资源完成时间；
3. 抓包看重传：HTTP/2 场景下一个丢包会导致大量流等待；观察 `Wireshark` 里 TCP 重传后"整段暂停"的时间窗；
4. 若环境丢包率低，对比应不显著甚至 HTTP/2 更快——验证"丢包是放大器"。

这也正是 HTTP/3/QUIC 的动机：**流级独立重传**，让丢包只堵自己那条流。
</Answer>
</Callout>
