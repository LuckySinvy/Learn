...
```http title="Diffie-Hellman：密钥从未传输，双方却算出同一个"
python3 -c '
import secrets
...
在 Playground 2 的基础上加入 Eve。关键点：Alice 和 Bob 之间**没有任何可信的身份锚点**，Eve 在两头各做一次 DH：

```python
# Eve 与 Alice 做一次 DH，得到 key_EA
a, e1 = secrets.randbits(256), secrets.randbits(256)
A, E1 = pow(g, a, p), pow(g, e1, p)
key_alice_to_eve = pow(E1, a, p)    # Alice 以为在跟 Bob 通话

# Eve 与 Bob 做一次 DH，得到 key_EB
b, e2 = secrets.randbits(256), secrets.randbits(256)
B, E2 = pow(g, b, p), pow(g, e2, p)
key_bob_to_eve = pow(E2, b, p)      # Bob 以为在跟 Alice 通话
```

结果：Alice 与 Eve 共享一把密钥，Eve 与 Bob 共享另一把。Alice 发来的密文用 `key_alice_to_eve` 解、再改用 `key_bob_to_eve` 重加密转发给 Bob，Bob 毫无察觉。**DH 只保证"协商出的密钥没人能算出来"，不保证"对面是谁"**——这正是证书要解决的：把公钥与身份绑定，让 Eve 无法冒充。没有证书时，中间人攻击畅通无阻。

**2. 证书过期为何导致签名验证失败**

用 Playground 3 的思路：证书内容是一串字节，CA 签名是对**这串字节**取摘要后加密的。验证时，浏览器会重新读取证书全部字段（**包括有效期**）算出摘要 B，与签名里解出的摘要 A 比较。

只要有效期字段变了（过期与否是有效期的一部分），证书的字节就变了 → 摘要 B 与签名时的摘要 A 不同 → 签名验证失败。所以**证书的任何字段（含有效期）被改动，签名都会失配**——签名保护的是"整个证书原封不动"，不是只保护域名和公钥。过期证书本质上等于"被改过的证书"，CA 对"过期后的内容"从未签过名。

**3. openssl s_client 观察**
...
- **证书链层级**：输出以 `Certificate chain` 开头的段落，`s:` 开头的是各层证书主体。典型三层：叶证书（`CN=example.com`）→ 中间 CA（如 `CN=DigiCert TLS RSA SHA256 2020 CA1`）→ 根 CA（`CN=DigiCert Global Root CA`）。`i:` 表示签发者，逐层配对就能看清信任链；
- **协商的 TLS 版本**：看 `Protocol : TLSv1.3`（或 1.2）。协议版本由客户端支持的套件与服务器配置共同决定，现代站点一般 TLS 1.3。

也可以加 `-showcerts` 看每层证书详情，`-verify_hostname` 校验域名匹配。

**4. 证书侧"网站提示不安全"的常见原因**

按证书验证的四个环节排查：

1. **域名不匹配**：证书的 SAN/CN 不含用户访问的域名。访问 `www.example.com` 但证书只签了 `example.com`（或反之），或用了裸 IP 访问只签域名的证书；
2. **证书过期**：有效期已过（最常见原因）。证书有效期通常 90 天（Let's Encrypt），自动续期脚本挂了就会这样；
3. **证书链不完整**：服务器只发了叶证书，没发中间 CA 证书，浏览器无法建立到预装根 CA 的信任链（"证书链不完整"报错）。中间证书漏配是 Let's Encrypt 用户的经典翻车点；
4. **算法/密钥问题**：证书签名算法已被禁用（如 SHA-1 签名证书、1024 位 RSA 过弱密钥）或私钥与证书不匹配；也可能是服务器用了自签名证书而系统不信任该 CA。

排查工具：`openssl s_client` 看链、`openssl x509 -text` 看有效期/SAN、`ssllabs.com` 做全面体检。
</Answer>
</Callout>
