...
<Callout type="tip" title="🤖 AI 时代，为什么还要学 HTTP？">
前后端之间的契约是 HTTP，无论 AI 写出多少封装好的请求代码，网络出问题、接口行为异常时，你都得回到协议本身去定位。理解请求/响应的真实结构、状态码的含义、缓存与鉴权头怎么工作，才能看穿 AI 生成的接口调用里那些隐蔽的错误。协议是调试网络世界的通用语言，AI 替代不了你用这门语言思考的能力。
</Callout>
...
把路径换成 `/nope.html` 后，状态行从 `200 OK` 变成 `404 Not Found`：

```http
HTTP/1.0 404 File not found
```

原因：`python3 -m http.server` 是静态文件服务器，找不到对应文件就回 404。注意 `curl` 默认不把 4xx 当错误（退出码仍是 0），要让它"感知"到失败需要加 `-f`（`--fail`）选项，失败时退出码变 22。

**2. HTTP/1.0 + 去掉 Host 头**

把请求行改成 `GET / HTTP/1.0` 并删掉 `Host` 头后，服务器**仍然会响应**（200），因为 HTTP/1.0 不要求 Host 头。而 HTTP/1.1 强制要求 Host，原因在于**虚拟主机**：

- 一台服务器（同一个 IP:端口）可以同时托管多个网站，服务器必须靠请求头里的 `Host` 字段判断"你想访问的是哪个站点"，才能返回对应的内容；
- 如果允许省略 Host，服务器就无法区分 `a.com` 和 `b.com` 的请求，虚拟主机机制就失效了。

所以 HTTP/1.1 把 Host 设为唯一强制必需的请求头（RFC 9112）。

**3. HTTPS 访问旅程图（标注四层）**

```text
① 输入网址、解析 URL          应用层（HTTP 语义，DNS 解析由系统库完成）
② DNS 把域名解析成 IP         应用层（DNS 协议，走 UDP 53 端口）
③ TCP 三次握手               传输层（建立可靠连接）
④ TLS 握手（证书验证、密钥交换） 应用层/表示层（TLS 位于 TCP 之上、HTTP 之下）
⑤ 发送 HTTP 请求（加密后）      应用层（HTTPS 报文经 TLS 加密）
⑥ 服务器返回加密响应            应用层
⑦ 浏览器渲染 + 子资源请求       应用层（复用同一连接发起更多请求）
⑧ 连接复用或关闭              传输层（keep-alive 决定去留）
```

其中每一步的比特最终都要经过网络层（IP 路由）和链路层（以太网帧）传输，只是这两层对 HTTP 透明，不需要应用关心。

**4. 扩展 curl -w 输出**

在 Playground 2 的基础上追加几个变量：
...
`-w` 的每个变量都是 curl 在一次请求过程中统计好的阶段指标，配合 `-o /dev/null` 可以只输出指标不显示响应体，非常适合做基准测试与监控。
</Answer>
</Callout>
