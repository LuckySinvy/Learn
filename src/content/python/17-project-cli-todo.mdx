...
`find` 方法只需一行列表推导，注意两边都做 `.lower()` 才是大小写不敏感：
...
完整可运行版（含 TodoStore 与 2 个测试，可直接替换练习 Playground 里的代码）：
...
class TestFind(unittest.TestCase):
    def setUp(self):
        self.path = Path(f"/tmp/todo_find_{id(self)}.json")
        self.store = TodoStore(self.path)
    def tearDown(self):
        if self.path.exists():
            self.path.unlink()
    ...
    def test_find_case_insensitive(self):
        ...
        self.store.add("Read Book")
        hits = self.store.find("milk")
        self.assertEqual(len(hits), 1)
        self.assertEqual(hits[0].title, "Buy Milk")

    def test_find_no_match(self):
        self.store.add("Buy Milk")
        self.assertEqual(self.store.find("xyz"), [])
...
- `keyword.lower() in t.title.lower()`：两边都小写化，实现大小写不敏感的子串匹配。
- 测试 1 验证 `"milk"` 能匹配到 `"Buy Milk"`；测试 2 验证无匹配时返回空列表。
- 每个测试用独立的临时 JSON 文件，`tearDown` 里清理，避免用例之间互相污染。
</Answer>
</Callout>
